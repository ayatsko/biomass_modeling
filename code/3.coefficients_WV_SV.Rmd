...
df$TLS_sum_biomass <- df$TLS_sum * df$PCT_wd

# remove rows with PCTID = 125
test <- df[df$PCTID != 125,]
ggplot(df, aes(x = ALS_sum, y = TLS_sum_biomass, color = as.factor(PCTID))) + 
  geom_point() +
  theme_classic()+
  geom_smooth(method = "lm", se = FALSE, color = "red") +
  labs(x = "ALS Shade Volume (m^3)", y = "TLS Woody Biomass (mass unit)") +
  theme(plot.title = element_text(hjust = 0.5))

mod <- lm(TLS_sum_biomass ~ ALS_sum, data = test)
summary(mod)
```
