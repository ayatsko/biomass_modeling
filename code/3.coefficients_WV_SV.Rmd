...
```{r}
cal <- read.csv("data/callitris.csv")
...
# testing PCT wd applied to volume 
df$TLS_sum_1m2_biomass <- df$TLS_sum_1m2 * (1000) * df$PCT_wd * 0.001  # final units are in kg

# remove rows with PCTID = 125
test <- df[df$PCTID != 125,]
ggplot(test, aes(x = ALS_sum_1m2, y = TLS_sum_1m2_biomass)) + 
  geom_point() +
  theme_classic()+
  geom_smooth(method = "lm", se = TRUE, color = "darkred") +
  labs(x = "Shade Volume (m^3)", y = "TLS Woody Biomass (kg)") +
  theme(plot.title = element_text(hjust = 0.5))

mod <- lm(TLS_sum_1m2_biomass ~ ALS_sum_1m2, data = test)
summary(mod)
```
