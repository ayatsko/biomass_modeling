...
tony_outputs <- read.csv("tony_output.csv")
```
...
test1 <- rbind(split0_30m, split1_30m)
```
...
tony_split1_2 <- tony_outputs[tony_outputs$name == "all_train_test_splits_test_0" | tony_outputs$name == "all_train_test_splits_test_1", ]
tony_split1_2$split <- substr(tony_split1_2$name, nchar(tony_split1_2$name), nchar(tony_split1_2$name))
tony_split1_2$grid <- 30

# select columns fid, predition_mean, ground_truth_mean
tony <- tony_split1_2[, c("fid", "ground_truth_mean", "prediction_mean", "grid", "split")]
...
ggplot(metrics_long, aes(x = Metric, y = Value, fill = person)) +
  ...
  labs(title = "Error Metrics by Height Class",
       ...
       y = "Error (m)", 
       fill = "Metric") +
  facet_wrap(~height_class, scales = "free")+
  theme_minimal()
