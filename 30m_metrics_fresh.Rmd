...
```{r}
outputs <- outputs %>%
  mutate(residuals = ground_truth_mean - prediction_mean)
...
calculate_metrics <- function(df) {
  n <- nrow(df)
  mae <- mean(abs(df$ground_truth_mean - df$prediction_mean), na.rm = TRUE)
  rmse <- sqrt(mean((df$ground_truth_mean - df$prediction_mean)^2, na.rm = TRUE))
  mbe <- mean(df$prediction_mean - df$ground_truth_mean, na.rm = TRUE)
  r2 <- summary(lm(prediction_mean ~ ground_truth_mean, data = df))$r.squared
  r <- cor(df$ground_truth_mean, df$prediction_mean, use = "complete.obs")
...
metrics_by_class <- outputs %>%
  ...
  summarise(MAE = mean(abs(ground_truth_mean - prediction_mean), na.rm = TRUE),
            RMSE = sqrt(mean((ground_truth_mean - prediction_mean)^2, na.rm = TRUE)),
            MBE = mean(prediction_mean - ground_truth_mean, na.rm = TRUE),
            R2 = summary(lm(prediction_mean ~ ground_truth_mean))$r.squared,
            Pearson_R = cor(ground_truth_mean, prediction_mean, use = "complete.obs"),
            MAPE = mean(abs((ground_truth_mean - prediction_mean) / ground_truth_mean) * 100, na.rm = TRUE)) # Add MAPE
...
```{r}
ggplot(metrics_long_all, aes(x = Metric, y = Value)) +
...
```{r}
ggplot(metrics_long, aes(x = height_class, y = Value)) +
  geom_boxplot() +  # Change to boxplo
  labs(title = "Mean Error Metrics",
       x = "Height class", 
       y = "Error") +
  ylim(0,100) +
  facet_wrap(~height_class, ncol = 3) +  # Facet by Metric
  theme_minimal()  # Minimal theme for a clean look
```
